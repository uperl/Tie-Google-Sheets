# Tie::Google::Sheets ![static](https://github.com/uperl/Tie-Google-Sheets/workflows/static/badge.svg) ![linux](https://github.com/uperl/Tie-Google-Sheets/workflows/linux/badge.svg)
...
(implemented by [Tie::Google::Sheets::Worksheet](https://metacpan.org/pod/Tie::Google::Sheets::Worksheet)) keyed by cell reference in
A1 notation (`A1`, `B12`, and so on), so that a spreadsheet can be read and
...
`client_email` address, the same way you would share it with any other
...
- spreadsheet\_id
    ...
    (`https://docs.google.com/spreadsheets/d/<id>/edit`). Required
    unless ["spreadsheet\_url"](#spreadsheet_url) is given instead.

- spreadsheet\_url

    The full URL of the spreadsheet, from which the spreadsheet id will be
    extracted. Ignored if ["spreadsheet\_id"](#spreadsheet_id) is also given.
...
- service\_account
    ...
    ["access\_token"](#access_token) is given instead.
...
- ua
    ...
    A user agent object (for example an [HTTP::Tiny](https://metacpan.org/pod/HTTP::Tiny) or [LWP::UserAgent](https://metacpan.org/pod/LWP::UserAgent)
    instance) to be wrapped in an [HTTP::AnyUA](https://metacpan.org/pod/HTTP::AnyUA). Defaults to a plain
    [HTTP::Tiny](https://metacpan.org/pod/HTTP::Tiny) instance.

- any\_ua

    An already constructed [HTTP::AnyUA](https://metacpan.org/pod/HTTP::AnyUA) compatible object (that is, anything
    providing a `request($method, $url, \%options)` method with the same
...
[perltie](https://metacpan.org/pod/perltie) for the full semantics of each method.

## TIEHASH

Constructor, called via `tie`; see ["CONSTRUCTOR"](#constructor) above.
...
must be `undef` or a hashref of cell reference / value pairs to populate
...
Together implement iteration (`keys`, `values`, `each`) over the
...
Creates a new worksheet named `$title`. If `\%cells` is given, its
key/value pairs are written into the new worksheet as cell reference /
value pairs. Returns the new worksheet hashref, the same as `$doc{$title}`.
...
- [Tie::Google::Sheets::Worksheet](https://metacpan.org/pod/Tie::Google::Sheets::Worksheet)
